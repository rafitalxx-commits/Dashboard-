            ...
            >
              ...
              <span className="day-events">
                ...
                {eventsForDay.slice(0, 2).map((ev) => (
                  <span
                    className={`event-chip ${ev.source === "task" ? "task" : ev.googleEventId ? "google" : "local"}`}
                    key={ev.id}
                    title={ev.title}
                  >
        ...
        <div className="day-panel">
          ...
          <div className="day-events-list">
            ...
            {selectedEvents.map((ev) => (
              <div className={`event-card ${ev.source === "task" ? "task" : ev.googleEventId ? "google" : "local"}`} key={ev.id}>
                <div className="event-card-header">
                  <span className={`event-indicator ${ev.source === "task" ? "task" : ev.googleEventId ? "google" : "local"}`} />
                  <span className="event-title">{ev.title}</span>
                </div>
...
.day.today {
  border-color: #38bdf8;
  background: #082f49;
  box-shadow: inset 0 0 0 1px rgba(56,189,248,0.55), 0 0 0 1px rgba(56,189,248,0.16);
}
.day.selected {
  background: #172554;
  border-color: #60a5fa;
...
.event-chip.task { background: rgba(250,204,21,0.18); color: #fde68a; }
...
.event-card.task { border-left: 3px solid #facc15; }
...
.event-indicator.task { background: #facc15; }
...
.input, select, textarea { background: #0f172a; color: inherit; border: 1px solid rgba(148,163,184,0.2); padding: 12px 14px; border-radius: 10px; font-size: 16px; min-width: 0; }
